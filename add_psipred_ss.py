...
if ext in available_formats:
    convert_alignment(config["reformat"], tmp["input_copy"], tmp['file_a3m'], ext, "a3m", ["-M "+str(param["M"])] if "M" in param else ["-M first"])
...
#convert alignment to regular fasta sequences (not aligned)
strip_alignment(tmp["input_copy"], tmp["file_fastaseqs"])
#make blast database
print('- '+str(time.strftime('%X %x'))+" creating BLAST database")
cmd_mkblastdb = subprocess.Popen([config["makeblastdb"], "-in", tmp["file_fastaseqs"], "-dbtype", "prot", "-out", filepath],
                                 stdout=subprocess.PIPE, stderr=subprocess.PIPE)
